...
void criar_txt_se_bin_existir() {
    ...
    if (!bin) {
        printf("Arquivo %s nao existe. Gere o arquivo binario primeiro.\n", origem);
        return;
    }
...
int main() {
    int tipo, quantidade;
    printf("Escolha o tipo de ordenacao para gerar o out.bin:\n");
    ...
    // So gera os arquivos .bin se nao existirem
    FILE *test = fopen("dados_asc.bin", "rb");

    if (!test) {
        printf("Arquivos binarios nao encontrados. Gerando todos...\n");
    } else {
        fclose(test);
    }
